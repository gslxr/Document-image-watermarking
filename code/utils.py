...
def random_blur_kernel(probs, blur_kernel, sigrange_gauss):
    N = blur_kernel

    coords = tf.to_float(tf.stack(tf.meshgrid(tf.range(N), tf.range(N), indexing='ij'), -1)) - (0.5 * (N - 1))

    # nothing, default
    value_nothing = tf.to_float(tf.reduce_sum(tf.abs(coords), -1) < 0.5)

    # gauss
    sig_gauss = tf.random.uniform([], sigrange_gauss[0], sigrange_gauss[1])  # [1.0,3.0)
    value_gauss = tf.exp(-tf.reduce_sum(coords ** 2, -1) / (0.5 * sig_gauss ** 2))

    value = tf.cond(tf.random_uniform([]) < probs, lambda: value_gauss, lambda: value_nothing)

    v = value / tf.reduce_sum(value)
    z = tf.zeros_like(v)
    filter_blur = tf.reshape(tf.stack([v, z, z, z, v, z, z, z, v], -1), [N, N, 3, 3])
